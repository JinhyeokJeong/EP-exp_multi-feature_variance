...
// function generating colored orientation texture
function drawTexture_colBars_corr(oriMean, hueMean, oriRan, hueRan, corrcond, coord, ctx){
    // is there efficient way to control each line's color?
    // draw orientations with certain colors in a grid

    // corrcond: 0 ~ independent, 1 ~ pos. corr, 2 ~ neg. corr
    // spatial: 0 ~ gradient (non-mixed), 1 ~ scattered (mixed)
    ...
    var ori_angles = get_angles(oriMean, oriRan, setSize); // get orientation values
    var hue_angles = get_angles(hueMean, hueRan, setSize); // get hue values    

    // manipulate correlation based on the value of corrcond
    if (corrcond==1){
        // positive correlation (a-b). Sort angles in the positive direction
        if (Math.random()>.5){
            // upper items have more CCW values
            ori_angles.sort(function(a,b){return a-b;}); // increasing trend
            hue_angles.sort(function(a,b){return a-b;}); // increasing trend
            flag_corr = 1.1;
        } else {
            // upper items have more CW values
            ori_angles.sort(function(a,b){return b-a;}); // decreasing trend
            hue_angles.sort(function(a,b){return b-a;}); // decreasing trend
            flag_corr = 1.2;
        }        
    } else if (corrcond==2) {
        // negative correlation (b-a). Sort angles in the negative direction
        if (Math.random()>.5){
            // upper items have more CCW orietnations but more CW hue angles.
            ori_angles.sort(function(a,b){return a-b;}); // increasing trend
            hue_angles.sort(function(a,b){return b-a;}); // decreasing trend
            flag_corr = 2.1;
        } else {
            // upper items have more CW orientations but more CCW hue angles.
            ori_angles.sort(function(a,b){return b-a;}); // decreasing trend
            hue_angles.sort(function(a,b){return a-b;}); // increasing trend
            flag_corr = 2.2;
        }
    ...
    // do sort (spatially aligned feature values)
    // manipulation may be different based on the correlation condition
    flag_spatial = 2;
    if (corrcond ==0){    
        if (Math.random()>.5){
            // sort only orientations
            which_sort = 'ori'
            if (Math.random()>.5){
                // CCW first
                ori_angles.sort(function(a,b){return a-b;})
                flag_spatial = 2.1;
            } else {
                // CW first
                ori_angles.sort(function(a,b){return b-a;})
                flag_spatial = 2.2;
            }                
        } else {
            which_sort = 'hue'
            ...
            if (Math.random()>.5){
                // CCW first
                hue_angles.sort(function(a,b){return a-b;})
                flag_spatial = 2.3;
            } else {
                // CW first
                hue_angles.sort(function(a,b){return b-a;})
                flag_spatial = 2.4;
            }                
        }
    }

    // convert ori angles into radian scale
    var ori_thetas = deg2rad(ori_angles);
    ...
    return {ori_angles: ori_angles, hue_angles: hue_angles, correlation: corrcond, which_sort: which_sort, flag_corr: flag_corr, flag_spatial: flag_spatial}
}